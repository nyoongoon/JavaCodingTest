...
//        System.out.println(solution(10, 13, times)); //490
    ...
    public static int solution(int delay, int capacity, int[] times) {
        int answer = 0;
        int[] arrivalTime = new int[times.length + 1];
        for (int i = 0; i < times.length; i++) {
            arrivalTime[i + 1] = arrivalTime[i] + times[i];
        }
        Queue<Integer> queue = new LinkedList<>();
        queue.add(arrivalTime[0]);
        int nowTime = 0;
        int idx = 0;
        ...
        while(!queue.isEmpty()){
            ...
            queue.poll();
            nowTime += 5;
            for (int i = idx; i < arrivalTime.length; i++) {
//                System.out.println(i);
                if(arrivalTime[i] <= nowTime){
                    if(queue.size() < capacity){
                        queue.add(arrivalTime[i]);
                    }else{
                        answer++;
                    }
